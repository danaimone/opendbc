...
// Experimental, opt-in startup preferences in Park or Drive. Does not replace factory
...
static bool subaru_startup_tx(const CANPacket_t *msg) {
  ...
  // Only the factory preference bits are writable, never brake/steering commands.
  // Reverse/neutral/manual gears wait; forward motion does not cancel startup.
  const bool driving_away = gear == 121U;
